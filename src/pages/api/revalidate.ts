/**
 * This code is responsible for revalidating the cache when an article is updated.
 *
 ...
 * 6. Filter: _type == "article" || _type == "settings"
...
type StaleRoute = '/' | `/article/${string}`
  ...
  if (body._type === 'article') {
    ...
    if (!exists) {
      ...
      if ((body.slug as any)?.current) {
        staleRoutes.push(`/article/${(body.slug as any).current}`)
      }
      // Assume that the article document was deleted. Query the datetime used to sort "More stories" to determine if the article was in the list.
      const moreStories = await client.fetch(
        groq`count(
          *[_type == "article"] | order(date desc, _updatedAt desc) [0...3] [dateTime(date) > dateTime($date)]
        )`,
        { date: body.date },
      )
      // If there's less than 3 articles with a newer date, we need to revalidate everything
      if (moreStories < 3) {
...
async function queryAllRoutes(client: SanityClient): Promise<StaleRoute[]> {
  ...
  return ['/', ...slugs.map((slug) => `/article/${slug}` as StaleRoute)]
}
...
async function mergeWithMoreStories(
  ...
  const moreStories = await client.fetch(
    groq`*[_type == "article"] | order(date desc, _updatedAt desc) [0...3].slug.current`,
  )
...
async function queryStaleArticleRoutes(
  ...
  let slugs = await client.fetch(
    groq`*[_type == "article" && _id == $id].slug.current`,
    { id },
  )
